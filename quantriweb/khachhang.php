...
$sql = "SELECT id, customer_name, customer_email, customer_phone, created_at, total_price ,status FROM orders";
$result = $conn->query($sql);

// Truy vấn để tìm khách hàng chi tiêu nhiều nhất đến ít nhất
$spender_sql = "SELECT 
    customer_name, 
    customer_email, 
    customer_phone, 
...
$spender_result = $conn->query($spender_sql);
...
<div class="containerdskh">
    ...
    <h4 class="text-center mt-5">Khách Hàng Mua Nhiều Nhất</h4>
<?php if ($spender_result->num_rows > 0): ?>
    <table class="table table-bordered">
        <thead>
        <tr>
            <th>Tên khách hàng</th>
        ...
        <tbody>
        <?php while ($spender = $spender_result->fetch_assoc()): ?>
            <tr>
                <td><?php echo $spender['customer_name']; ?></td>
                <td><?php echo $spender['customer_email']; ?></td>
                <td><?php echo $spender['customer_phone']; ?></td>
                <td><?php echo number_format($spender['total_spent'], 2); ?></td>
            </tr>
        <?php endwhile; ?>
        </tbody>
    </table>
