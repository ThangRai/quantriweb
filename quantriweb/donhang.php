...
$sql = "SELECT orders.id, orders.product_id, orders.quantity, orders.total_price, orders.customer_name, customer_phone, orders.customer_email, orders.customer_address, orders.created_at, orders.status, products.name AS product_name
        FROM orders
        LEFT JOIN products ON orders.product_id = products.id";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr id='order_" . $row['id'] . "'>
                ...
                <td>
                    <form method='POST' action=''>
                        <select name='status' onchange='this.form.submit()'>
                            ...
                            <option value='Đã thanh toán' " . ($row['status'] == 'Đã thanh toán' ? 'selected' : '') . ">Đã thanh toán</option>
                            <option value='Đã di chuyển' " . ($row['status'] == 'Đã di chuyển' ? 'selected' : '') . ">Đã di chuyển</option>
                ...
                <td>
                    <button class='btn btn-danger delete_order' data-id='" . $row['id'] . "'>
...
// Kiểm tra khi admin thay đổi trạng thái đơn hàng
if (isset($_POST['status']) && isset($_POST['order_id'])) {
    $new_status = $_POST['status'];
    $order_id = $_POST['order_id'];
    
    // Kiểm tra trạng thái đã thay đổi thực sự hay không
    if (!empty($new_status)) {
        ...
        if ($conn->query($update_sql) === TRUE) {
            ...
            echo "<script>window.location.href = window.location.href;</script>"; // Reload trang sau khi cập nhật trạng thái
        } else {
            echo "Lỗi: " . $conn->error;
        }
    }
...
<!-- Thêm phần script xử lý AJAX -->
<script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>

<script>
    $(document).on('click', '.delete_order', function() {
        var orderId = $(this).data('id');
        var row = $(this).closest('tr'); // Lấy row hiện tại để xoá

        if (confirm('Bạn có chắc chắn muốn xóa đơn hàng này không?')) {
            $.ajax({
                url: 'delete_order.php', // Đường dẫn tới file xử lý xoá
                type: 'POST',
                data: { order_id: orderId },
                success: function(response) {
                    row.remove(); // Xoá row khỏi table
                    alert('Đơn hàng đã bị xoá!');
                },
                error: function() {
