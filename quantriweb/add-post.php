...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $noidung = isset($_POST['noidung']) ? htmlspecialchars($_POST['noidung']) : ''; // Kiểm tra tồn tại
    $image_url = ""; // Khởi tạo biến đường dẫn hình ảnh
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (in_array($imageFileType, $allowed_types)) {
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
                ...
                echo '<div class="alert alert-error">Không thể lưu hình ảnh. Vui lòng kiểm tra quyền thư mục.</div>';
            }
        } else {
            echo '<div class="alert alert-error">Chỉ hỗ trợ định dạng ảnh JPG, JPEG, PNG, GIF.</div>';
        }
    }

    $sql = "INSERT INTO posts (title, author, content, noidung, image_url) VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssss", $title, $author, $content, $noidung, $image_url);

    if ($stmt->execute()) {
        echo '<div class="alert alert-success">Bài viết đã được thêm thành công.</div>';
    } else {
        echo '<div class="alert alert-error">Lỗi: ' . $stmt->error . '</div>';
    }
}
?>
