...
// Truy vấn để lấy lịch sử truy cập từ bảng truycap
$sql = "SELECT username, access_time, browser FROM truycap ORDER BY access_time DESC";
$result = $conn->query($sql);
?>

    <div class="containerlstruycap">
        <h2 class="text-center mb-4">Lịch Sử Truy Cập</h2>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $access_time = date('d/m/Y H:i:s', strtotime($row['access_time']));
                $browser = $row['browser']; // Trình duyệt người dùng
                echo '<tr>';
                echo '<td>' . htmlspecialchars($row['username']) . '</td>';
                echo '<td>' . "Đăng nhập lúc " . $access_time . '</td>';
                echo '<td>' . "bằng " . htmlspecialchars($browser) . '</td>';
                echo '</tr>';
            }
            ...
            echo '<p class="alert alert-warning">Chưa có lịch sử truy cập nào.</p>';
        }
