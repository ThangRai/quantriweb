...
// Xử lý khi người dùng nhấn nút "Thêm Menu"
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $menu_name = $_POST['menu_name']; // Tên menu
    $menu_link = $_POST['menu_link']; // Liên kết menu

    // Thêm menu vào bảng menu
    $sql = "INSERT INTO menus (menu_name, menu_link) VALUES (?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ss", $menu_name, $menu_link);
    if ($stmt->execute()) {
        echo "<script>alert('Thêm menu thành công!'); window.location.href='them_menu.php';</script>";
    } else {
        echo "<script>alert('Lỗi: Không thể thêm menu.');</script>";
    }
    $stmt->close();
}
$conn->close();
    ...
    <div class="container">
        ...
        <form method="POST" action="">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="menu_name" name="menu_name" placeholder="Nhập tên menu..." required>
            </div>
            <div class="form-group">
                <label for="menu_link">Liên Kết Menu</label>
                <input type="text" class="form-control" id="menu_link" name="menu_link" placeholder="Nhập liên kết menu (VD: /gioithieu.php)" required>
            </div>
        ...
        <div class="footer">
            <p>&copy; 2024 <a href="#">Quản Trị Web</a>. All rights reserved.</p>
        </div>
    </div>
</body>
</html>
