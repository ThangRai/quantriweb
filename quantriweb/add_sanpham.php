...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Kiểm tra dữ liệu và thêm sản phẩm vào CSDL
    if (empty($name) || empty($original_price) || empty($current_price) || empty($image_name)) {
        $error = "Vui lòng điền đầy đủ thông tin.";
    } else {
        $sql = "INSERT INTO products (name, original_price, current_price, image) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sdss", $name, $original_price, $current_price, $image_folder);
        if ($stmt->execute()) {
...
?>
<style>
    /* Cài đặt font chữ và khoảng cách cho body */
        body {
        ...
        /* Bố trí form theo flexbox với 2 cột */
        .form-row {
            display: flex;
        ...
        /* Responsive thiết kế cho màn hình nhỏ hơn */
        @media (max-width: 768px) {
            .form-container {
                width: 90%; /* Chiều rộng giảm xuống cho màn hình nhỏ */
            }
            
            /* Chuyển đổi form thành 1 cột trên màn hình nhỏ */
            .form-row {
...
<div class="form-container">
    ...
    <!-- Form thêm sản phẩm -->
    <form action="add_sanpham.php" method="POST" enctype="multipart/form-data">
        <!-- Sử dụng form-row để tạo 2 cột -->
        <div class="form-row">
